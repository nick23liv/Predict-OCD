...
Produces two figures exploring longitudinal raw cognitive trajectories:
  ...
  Figure 1 (trajectories.png)
    ...
    – Population OLS trend + 95 % CI overlaid.
  ...
  Figure 2 (baseline_vs_slope.png)
    – Scatter: each participant's baseline (ses-00A) score vs their
      individual rate of change (OLS slope, points/year).
...
# ── Aesthetics ─────────────────────────────────────────────────────────────────
# Colourblind-friendly Tableau palette, one colour per domain
DOMAIN_COLORS = {
    "IQ":              "#4e79a7",   # blue
...
# Sequential colours for the 4 waves (light → dark)
WAVE_COLORS = {
    "ses-00A": "#b8d4e8",
    "ses-02A": "#6baed6",
    "ses-04A": "#2171b5",
...
# Each entry defines the column to plot, display labels, and any row filter.
DOMAINS = {
    "IQ": {
        "col":    "IQ_scaled_notz",
        "title":  "IQ\n(WISC-V Scaled Score)",
    ...
    "RespInhib": {
        ...
        "filter": ("task_type", "nihtb_flanker"),  # column, keep-value
    },
    "CogFlex": {
...
# One age value per participant × session (take the first non-null)
age_df = (
...
def ols_with_ci(x, y, n_points=200, ci=0.95):
    """Return x_range, y_pred, y_lo, y_hi for a simple OLS fit."""
...
# ── Helper: per-participant baseline + slope ───────────────────────────────────
def compute_baseline_slope(data, col):
    """
    Returns a DataFrame with one row per participant:
      baseline  – score at ses-00A (NaN if missing)
      slope     – OLS slope of score ~ age (NaN if < 2 timepoints)
      n_waves   – number of non-null timepoints used
    """
    records = []
    for pid, grp in data.groupby("participant_id"):
        grp = grp.dropna(subset=[col, "age"]).sort_values("age")
...
fig1.suptitle(
    ...
    "Grey lines = individual participants; coloured line = population OLS trend ± 95 % CI; "
...
for ax_idx, (domain_key, domain) in enumerate(DOMAINS.items()):
    ...
    # Apply row filter (e.g. nihtb only for RespInhib)
    data = df.copy()
    if domain["filter"]:
        fcol, fval = domain["filter"]
        data = data[data[fcol] == fval]

    # Working subset: drop rows with no score or no age
    plot_df = data[["participant_id", "session_id", "age", col]].dropna(subset=[col, "age"])

    # ── Spaghetti: one thin grey line per participant ──────────────────────
    n_drawn = 0
    ...
    # ── Scatter dots coloured by wave ─────────────────────────────────────
    for wave in ["ses-00A", "ses-02A", "ses-04A", "ses-06A"]:
...
# ── Shared wave legend in the empty 6th panel ────────────────────────────────
...
fig2.suptitle(
    ...
    "Each dot = one participant. Slope = OLS coefficient (score units / year) "
    "from their personal trajectory (≥ 2 waves required).\n"
...
# Collect r-values for the summary bar chart
r_summary = {}
...
for ax_idx, (domain_key, domain) in enumerate(DOMAINS.items()):
    ...
    # ── Horizontal zero-slope reference ───────────────────────────────────
    ...
    ax.set_xlabel(f"Baseline score (ses-00A)\n{domain['ylabel']}", labelpad=4)
    ax.set_ylabel("Individual slope\n(score units / year)", labelpad=4)
    ax.set_title(domain["title"], fontweight="bold", pad=8)
...
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Baseline–slope Pearson r summary")
print("=" * 60)
